...
The `.cursorrules` file in the `cursor-setup/` directory is automatically loaded by Cursor. You don't need to do anything special - just open this repository in Cursor and start using the skills.
...
. *.cursorrules file* (in cursor-setup/)
+
...
. Update `.cursorrules` if adding new trigger commands
. Test in Cursor
. Commit changes
...
. Add trigger commands to `.cursorrules`
