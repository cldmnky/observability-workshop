...
logging-collector-xxxxx                        2/2     Running   0          1h
logging-collector-xxxxx                        2/2     Running   0          1h
...
----
+
NOTE: Depending on your Logging release, collector pods may appear as `logging-collector-xxxxx`, `collector-xxxxx`, or `instance-xxxxx`.
...
Each service writes structured request logs to stdout, which Loki collects via the cluster-logging pipeline. In Module 3 you will enable the OpenTelemetry SDK, which adds a second logging pathway: application-level logs are emitted through the OTel log SDK (correlated with the active trace), while HTTP access logs continue to be written as JSON lines to stdout — making them distinguishable in LokiStack by format and source.
...
[source,text,role="copypaste"]
----
{"time":"2026-09-17T13:54:00.784803374Z","level":"INFO","msg":"access","service":"frontend","method":"GET","path":"/api/notes","route":"/api/notes","status":200,"duration_ms":180,"bytes":1568495,"remote_addr":"10.232.1.120:33798"}
2026/09/17 13:54:00 INFO proxied request to backend method=POST path=/api/notes backend_status=201 target=http://backend:8081/api/notes
{"time":"2026-09-17T13:54:01.789533649Z","level":"INFO","msg":"access","service":"backend","method":"POST","path":"/api/notes","route":"/api/notes","status":201,"duration_ms":93,"bytes":123,"remote_addr":"10.232.1.117:33544"}
2026/09/17 13:54:01 INFO proxied to database route=/notes method=POST http_status=201
{"time":"2026-09-17T13:54:02.586475925Z","level":"INFO","msg":"access","service":"database","method":"GET","path":"/notes","route":"/notes","status":200,"duration_ms":89,"bytes":1569723,"remote_addr":"10.232.1.118:34302"}
{"time":"2026-09-17T13:53:59.257811716Z","level":"INFO","msg":"access","service":"frontend","method":"GET","path":"/not-found","route":"/","status":404,"duration_ms":0,"bytes":22,"remote_addr":"10.232.1.120:33816"}
----
+
Each service emits two log styles: JSON access logs (`{"msg":"access",...,"status":...,"duration_ms":...}`) via the stdout access-log middleware, and plaintext operational logs (`INFO proxied request...`, `INFO event created...`). Both are collected by Loki — the queries below handle the JSON access logs, which carry the `status` and `duration_ms` fields.
+
...
* ✓ `frontend` logs include `/ping` (200) and `/not-found`, `/api/does-not-exist`, `/broken` (404) from the load generator
...
=== How a LogQL query works (30 seconds)
...
* **Selection in `{ }` picks which logs to look at.** Loki only indexes labels such as namespace and pod name — not the message text. The part in braces (for example, `{kubernetes_namespace_name="...-observability-demo"}`) tells Loki which stored streams to fetch. Keep it narrow: fewer streams means a faster query.
* **Pipes after `{ }` process those logs line by line, left to right.** Each `| ...` stage takes the lines from the previous stage and either filters them (`|=`, `|~`, `| status=...`), parses them (`|json`), or reshapes them (`| line_format`). If a stage filters a line out, later stages skip it.
* **Parsing happens at query time.** The message text itself is not indexed. When you write `|json`, Loki extracts temporary labels from the log content just for this query ("schema on read"). Formatting stages only change what you see — never the stored data.

Example: `{kubernetes_namespace_name="..."} |json | line_format "{{.message}}" |json | status=404` means "fetch my namespace's streams, parse the collector envelope, keep only the message text, parse that JSON too, and show only lines where the parsed status is 404".

=== LogQL operators you'll use

Read through this short primer first — the queries below reuse the same few operators. Each query selects log streams with labels, then narrows or reshapes lines with pipeline stages:

* `{label="value"}` — select log streams. Example: `{kubernetes_namespace_name="...-observability-demo"}` selects all logs from your namespace.
* `kubernetes_pod_name=~"frontend.*"` — the `=~` operator matches a regular expression, here any pod starting with `frontend`.
* `|= "text"` / `!="text"` — keep or drop lines containing a substring. Chaining two `|=` filters means logical AND (both must match).
* `|~ "regex"` — keep lines matching a regular expression anywhere in the line.
* `|json` — parse a JSON log line into labels. You'll use it twice: first for the collector's JSON envelope, then (after `line_format`) for the inner JSON access log, which exposes `method`, `path`, `status`, and `duration_ms` as labels.
* `| line_format "{{.message}}"` — replace the log line with a template, here just the contents of the `message` field.
* `| status=404` — after parsing, keep only lines where the parsed `status` label equals 404. This is precise; a bare `|= "404"` would also match ports like `:40456` or pod IDs.
* `rate()`, `avg_over_time()`, `| unwrap <field>` — turn log lines into numbers. `| unwrap duration_ms` promotes the parsed `duration_ms` label to a sample value so `avg_over_time(... [5m])` can average it. You'll see the full three-stage pattern (parse → unwrap → aggregate) in the last query.
* `| __error__ = ""` — keep only lines that parsed cleanly. If `|json` can't parse a line (for example, a plaintext `INFO proxied...` line where you expected JSON), Loki keeps the line but tags it with `__error__`. Adding this filter hides those parse failures; `| __error__ != ""` shows only the failures when debugging.

With that vocabulary in place, work through the queries below. Short reminders repeat the key idea under each query. When a double-`|json` query returns fewer lines than expected, remember the second `|json` skips plaintext operational logs — they get an `__error__` tag instead of `status`/`duration_ms` labels.

=== Steps

. Access the OpenShift console and navigate to **Observe** → **Logs**
(URL path: `/observe/logs`)
+
This opens the log query interface powered by Loki.
+
[NOTE]
====
The direct URL path for the Logs interface is `/observe/logs`. Use the left navigation menu: **Observe** → **Logs** to reach the log query interface.
====
...
[source,logql,role="copypaste"]
----
{kubernetes_namespace_name="%OPENSHIFT_USERNAME%-observability-demo"} |json | line_format "{{.message}}" |json | status=404
----
+
The double-`|json` pattern from the primer: envelope first, then inner access log. `| status=404` keeps only true 404 responses.
+
[NOTE]
====
**Why not just use** `|= "404"`**?**

The `|=` operator matches the string `404` anywhere in the line — including ports like `:40456`, pod IDs like `fd1404a3-...`, or byte counts. To filter accurately on the HTTP status code, you must:

. Parse the collector envelope (`|json`), extract `message` (`| line_format`)
. Parse the inner access-log JSON (second `|json`)
. Filter on the parsed label (`| status=404`)

This returns only true 404 responses such as `{"path":"/not-found","status":404,...}`.
====
+
...
----
+
The `=~` regex selector from the primer — any pod starting with `frontend`.
...
This shows note activity (any line containing `/api/notes`) from the frontend route.

. Combine multiple filters to find successful frontend requests:
+
[source,logql,role="copypaste"]
----
{kubernetes_namespace_name="%OPENSHIFT_USERNAME%-observability-demo", kubernetes_pod_name=~"frontend.*"} |json | line_format "{{.message}}" |json | status=200 |= "/ping"
----
+
As in the primer: `| status=200` plus `|= "/ping"` means successful responses AND the ping endpoint.

This shows successful `/ping` requests (HTTP 200) from the frontend pod, such as `{"path":"/ping","status":200,...}`.

. Use LogQL to count error rates:
+
[source,logql,role="copypaste"]
----
sum(rate({kubernetes_namespace_name="%OPENSHIFT_USERNAME%-observability-demo"} |~ `(?:status\s*=\s*404|\\?"status\\?"\s*:\s*404)` [5m]))
----
+
This calculates the rate of 404 errors per second over the last 5 minutes, using the `|~` regex filter from the primer. It matches `status=404` or `"status":404`, so it counts 404s whether the access log is plain text or JSON.
...
----
+
As in the primer: `|json` exposes `message` and metadata labels, then `line_format` templates the display line.
...
----
+
The parse → unwrap → aggregate pattern from the primer, applied to frontend response time. `avg by (kubernetes_pod_name)` keeps one series per pod to stay under Loki's per-query series limit.
...
**What you learned**: LogQL uses labels (like namespace, pod name) to select log streams, then filters content with operators like `|=` (contains substring), `|~` (contains regex match), and `!=` (does not contain). Parsing and formatting stages (`|json`, `| regexp`, `| line_format`) extract or reshape structured fields, and `| unwrap` unlocks metric-style aggregations over those fields.
...
. For the `|~` rate query, verify the regex matches your log format (`status=404` or `"status":404`)
. For `| status=404` queries, ensure the double-`|json` pipeline is used: `|json | line_format "{{.message}}" |json`
. Allow 5–10 minutes for ingestion under load — the `1x.extra-small` LokiStack throttles with `429 Too Many Requests` when many users query simultaneously (check `oc logs -n openshift-logging -l app.kubernetes.io/component=collector --tail=50` for `429` warnings). If the newest logs in Loki are older than `oc logs`, expand the query time range to last 2 hours.
...
----
+
You should see a traffic while the error generator is running.
...
----
+
This shows the true 404 error responses during the spike (for example, `{"path":"/not-found","status":404,...}`). The double-`|json` pipeline is required because the access log is JSON nested inside the collector's JSON envelope.
...
----
+
This parses the envelope, then the inner access-log JSON, keeps only 404s, and reformats each line as `GET /not-found -> 404` using the parsed `method`, `path`, and `status` labels.
...
image::dotted-kube.png[OpenShift correlation view in OpenShift Console]
...
**Issue**: Cannot extract structured data

**Solution**:
. Verify log format: access logs are JSON (`{"method":"GET","path":"/...","status":404,...}`), operational logs are plaintext (`INFO proxied request...`). The `|json | line_format "{{.message}}" |json` pipeline only extracts `method`, `path`, `status`, `duration_ms` from JSON access logs.
. View raw logs first: `{kubernetes_namespace_name="%OPENSHIFT_USERNAME%-observability-demo"}`
. Test the double-`|json` pipeline step by step: first `|json`, then add `| line_format "{{.message}}"`, then second `|json`
...
spec:
  ...
  - name: application-errors
    ...
    - alert: HighApplicationErrorRate
      expr: |
        sum(rate({kubernetes_namespace_name="%OPENSHIFT_USERNAME%-observability-demo"} |json | line_format "{{.message}}" |json | status=404 [5m])) > 0.5
      for: 2m
      labels:
        severity: warning
        team: platform
...
The `expr` uses the same double-`|json` pipeline you used in Exercise 3: parse the collector envelope, parse the inner access-log JSON, then count only lines where `status=404`. When the per-second rate stays above 0.5 for 2 minutes, the alert fires. A bare `|= "404"` would match ports, pod IDs, and byte counts (about 26/s of noise) — the parsed filter returns the true 404 rate (about 0.8/s from the load generator).
...
. Create the AlertingRule by applying the YAML shown above:
+
[source,bash,role="copypaste"]
----
cat <<EOF | oc apply -f -
...
----
+
TIP: You can also import this YAML through the OpenShift web console by navigating to the **+** (Import YAML) button in the top-right corner, pasting the YAML, and clicking **Create**.
...
You got a first idea of what centralized logging makes possible. Instead of accessing individual pods only, you tried out:

* Centralized logs from multiple microservices in 1 queryable system
* Searching across services with LogQL
* Comparing metric spikes with log messages
* An example of automated alerts on log patterns
...
This module gave you a first look at centralized logging with LokiStack.

**What you explored**:

* Verified the LokiStack logging infrastructure deployed via GitOps
* Generated application logs across multiple services to query
* Tried out LogQL queries to filter and search logs
* Compared metric spikes with log messages for faster root cause analysis
* Saw an overview of how LokiStack's ruler and Alertmanager work together for log-based alerting
...
**LogQL operators introduced**:

* `{label="value"}`: Select log streams by label
* `|=`: Filter for lines containing text
* `!=`: Exclude lines containing text
* `|~`: Regex contains filter (matches a regex anywhere in the line)
* `|json`: Parse the collector's JSON envelope into labels
* `| line_format`: Replace the log line with a template (for example, just `{{.message}}`)
