...
[source,bash,role="copypaste"]
----
oc run -n {OPENSHIFT_USERNAME}-observability-demo traffic-generator --rm -i --restart=Never --image=quay.io/curl/curl:8.11.1 -- \
...
* ✓ Multiple applications are running in the {OPENSHIFT_USERNAME}-observability-demo namespace
