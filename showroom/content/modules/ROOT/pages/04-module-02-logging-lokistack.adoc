...
[source,logql,role="copypaste"]
----
{kubernetes_namespace_name="%OPENSHIFT_USERNAME%-observability-demo"} |json |line_format "{{.message}}" |= "404"
----
+
The `|json` stage first parses the collector's JSON envelope. The `| line_format "{{.message}}"` stage then replaces the log line with just the contents of the `message` field. Finally, `|= "404"` keeps only lines where that message contains the string `404`.
+
...
[source,logql,role="copypaste"]
----
{kubernetes_namespace_name="%OPENSHIFT_USERNAME%-observability-demo", kubernetes_pod_name=~"frontend.*"} |= "/ping" |= "200" |json
----
+
Chaining two `|=` filters requires both substrings to be present (logical AND). Here the line must contain both `/ping` and `200`. The trailing `|json` then parses the collector's JSON envelope for display.

This shows successful `/ping` requests (HTTP 200) from the frontend pod.
...
. Verify the `|~` regex matches the `status` field in your log format (`status=404` or `"status":404`)
...
[source,logql,role="copypaste"]
----
{kubernetes_namespace_name="%OPENSHIFT_USERNAME%-observability-demo"} |json | regexp `status=(?P<status>\d{3})` | status="404"
----
+
This shows the 404 error responses during the spike.
...
The `expr` uses the same `|= "404"` substring filter you used in Exercise 3: when the per-second rate of log lines containing `404` stays above 0.5 for 2 minutes, the alert fires.
