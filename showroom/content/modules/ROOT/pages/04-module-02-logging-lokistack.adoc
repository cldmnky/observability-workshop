...
[source,bash]
----
oc api-resources | grep 'shipwright.io'
----
+
...
[source,bash]
----
oc wait --for=condition=Succeeded -n SHOWROOM_USERNAME-observability-demo buildrun/frontend-buildrun --timeout=20m
oc wait --for=condition=Succeeded -n SHOWROOM_USERNAME-observability-demo buildrun/backend-buildrun --timeout=20m
oc wait --for=condition=Succeeded -n SHOWROOM_USERNAME-observability-demo buildrun/database-buildrun --timeout=20m
----
+
[NOTE]
====
This will build container images for the frontend, backend, and database services using Shipwright. It may take several minutes to complete. If the deployment fails with an image pull error, try a `oc rollout restart` on the affected deployment after the build completes.
====
+
...
[source,bash]
----
oc get pods -n SHOWROOM_USERNAME-observability-demo -l app=frontend
oc get pods -n SHOWROOM_USERNAME-observability-demo -l app=backend
oc get pods -n SHOWROOM_USERNAME-observability-demo -l app=database
----
...
[source,bash]
----
oc get route frontend -n SHOWROOM_USERNAME-observability-demo
FRONTEND_HOST=$(oc get route frontend -n SHOWROOM_USERNAME-observability-demo -o jsonpath='{.spec.host}')
...
[source,bash]
----
oc run -n SHOWROOM_USERNAME-observability-demo traffic-generator --rm -i --restart=Never --image=quay.io/curl/curl:8.11.1 -- \
...
[source,bash]
----
oc logs -n SHOWROOM_USERNAME-observability-demo deployment/frontend --tail=20
oc logs -n SHOWROOM_USERNAME-observability-demo deployment/backend --tail=20
oc logs -n SHOWROOM_USERNAME-observability-demo statefulset/database --tail=20
----
+
.Sample output
[source,text]
...
* ✓ Multiple applications are running in the SHOWROOM_USERNAME-observability-demo namespace
...
Click **Run query**
+
You should see log entries from all pods in the SHOWROOM_USERNAME-observability-demo namespace.
+
...
[source,logql]
----
{kubernetes_namespace_name="SHOWROOM_USERNAME-observability-demo"} |= "404" |json
----
+
...
[source,logql]
----
{kubernetes_namespace_name="SHOWROOM_USERNAME-observability-demo", kubernetes_pod_name=~"frontend.*"} |json
----
+
...
[source,logql]
----
{kubernetes_namespace_name="SHOWROOM_USERNAME-observability-demo", kubernetes_pod_name=~"frontend.*"} |= "/api/notes" |json
----
...
[source,logql]
----
{kubernetes_namespace_name="SHOWROOM_USERNAME-observability-demo", kubernetes_pod_name=~"frontend.*"} |= "path=/ping" |= "status=200" |json
----
+
...
[source,logql]
----
sum(rate({kubernetes_namespace_name="SHOWROOM_USERNAME-observability-demo"} |= "404" [5m]))
----
+
...
[source,logql]
----
{kubernetes_namespace_name="SHOWROOM_USERNAME-observability-demo"} |json | line_format "{{.kubernetes_pod_name}}: {{.message}}"
----
+
...
avg_over_time(
  {
    kubernetes_namespace_name="SHOWROOM_USERNAME-observability-demo",
...
* ✓ Logs from SHOWROOM_USERNAME-observability-demo namespace are visible
...
. Verify pods are running: `oc get pods -n SHOWROOM_USERNAME-observability-demo`
...
. Use quotes around label values: `{kubernetes_namespace_name="SHOWROOM_USERNAME-observability-demo"}`
...
[source,bash]
----
oc run -n SHOWROOM_USERNAME-observability-demo error-generator --rm -i --restart=Never --image=quay.io/curl/curl:8.11.1 -- \
...
[source,promql]
----
sum(rate(container_network_receive_bytes_total{namespace="SHOWROOM_USERNAME-observability-demo",pod=~"frontend-.*"}[5m]))
----
+
...
[source,logql]
----
{kubernetes_namespace_name="SHOWROOM_USERNAME-observability-demo"} |= "404" |= "error"
----
+
...
. View raw logs first: `{kubernetes_namespace_name="SHOWROOM_USERNAME-observability-demo"}`
...
[NOTE]
====
For the `application` tenant, AlertingRules must be created *in your application namespace*, not in `openshift-logging`.
The namespace must carry the label `openshift.io/loki-rules: "true"`, which the workshop pre-provisions.
====
+
[source,bash]
----
...
spec:
  ...
  - name: application-errors
    ...
    - alert: HighApplicationErrorRate
      expr: |
        sum(rate({kubernetes_namespace_name="SHOWROOM_USERNAME-observability-demo"} |= "404" [5m])) > 0.5
      for: 2m
      labels:
        severity: warning
        team: platform
      annotations:
        summary: "High error rate in SHOWROOM_USERNAME-observability-demo namespace"
        description: "Application in SHOWROOM_USERNAME-observability-demo is producing more than 0.5 errors/second for 2 minutes."
EOF
----
+
...
[source,bash]
----
oc get alertingrule -n SHOWROOM_USERNAME-observability-demo
----
...
[source,bash]
----
oc run -n SHOWROOM_USERNAME-observability-demo alert-generator --rm -i --restart=Never --image=quay.io/curl/curl:8.11.1 -- \
...
. Verify namespace: for `application` tenant, AlertingRules must be in your application namespace (e.g. `SHOWROOM_USERNAME-observability-demo`), not `openshift-logging`
. Verify the namespace has the `openshift.io/loki-rules=true` label: `oc get namespace SHOWROOM_USERNAME-observability-demo --show-labels`
. Check for syntax errors: `oc describe alertingrule high-error-rate -n SHOWROOM_USERNAME-observability-demo`
...
. Check Loki rules: `oc get alertingrule -n SHOWROOM_USERNAME-observability-demo`
