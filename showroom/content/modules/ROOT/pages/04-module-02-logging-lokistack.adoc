...
----
+
The first `|json` parses the collector's JSON envelope. `| line_format "{{.message}}"` replaces the log line with just the inner `message` string (which is itself JSON access-log). The second `|json` parses that inner JSON into labels such as `method`, `path`, `status`, and `duration_ms`. Finally, `| status=404` keeps only lines where the parsed `status` field equals 404.
+
[NOTE]
====
...
This shows note creation and note update calls (any line containing `/api/notes`) from the frontend route. The `|=` filter narrows the stream first, then `|json` parses the collector's JSON envelope for display.
...
----
+
The double-`|json` pipeline parses the envelope, then the inner access-log JSON. `| status=200` keeps only successful responses, and `|= "/ping"` narrows further to the ping endpoint (logical AND).
...
----
+
This calculates the rate of 404 errors per second over the last 5 minutes. The `|~` stage is a regex contains filter applied directly to the raw log line. The pattern matches either `status=404` (logfmt style, with optional spaces around `=`) or `"status":404` (JSON style, with optional escaping and optional spaces around `:`), so it counts 404s whether the access log is emitted as plain text or as JSON.
...
----
+
The first `|json` parses the collector's JSON envelope. `| line_format "{{.message}}"` exposes the inner access-log JSON string. The second `|json` parses that inner JSON, extracting numeric fields such as `duration_ms`, `status`, `method`, and `path`. `| unwrap duration_ms` promotes it to a numeric sample value. The outer `avg by (kubernetes_pod_name)` aggregates across all the extracted metadata labels, collapsing the result to one series per pod — which avoids hitting Loki's per-query series limit. `avg_over_time` computes the average frontend response time in milliseconds over the last 5 minutes, without requiring a separate Prometheus metric.
