...
----
+
The `|json` stage first parses the collector's JSON envelope. The `| regexp` stage then extracts the `status` field from the structured log message. Finally, `| status="404"` filters for only logs where the parsed status equals "404".
+
[NOTE]
====
**Why not use** `|= "404"`**?**

The `|=` operator performs text matching on the entire log line, which can match "404" appearing anywhere - in paths like `/404.html`, timestamps, or other fields. To filter accurately on the HTTP status code, you must:

1. Parse the log structure first (`|json`, then `| regexp`)
2. Extract the `status` field as a label
3. Filter on the parsed label (`| status="404"`)

This ensures you only see logs where the actual status code is 404, not just logs containing the string "404".
====
...
[source,logql,role="copypaste"]
----
sum(rate({kubernetes_namespace_name="%OPENSHIFT_USERNAME%-observability-demo"} |json | regexp `status=(?P<status>\d{3})` | status="404" [5m]))
----
+
This calculates the rate of 404 errors per second over the last 5 minutes. The query parses the log structure and filters on the actual `status` field to ensure accurate counting.
...
. Verify filter correctly parses and matches the status field from structured logs
...
spec:
  ...
  - name: application-errors
    ...
    - alert: HighApplicationErrorRate
      expr: |
        sum(rate({kubernetes_namespace_name="%OPENSHIFT_USERNAME%-observability-demo"} |json | regexp `status=(?P<status>\d{3})` | status="404" [5m])) > 0.5
      for: 2m
      labels:
        severity: warning
        team: platform
