...
[source,text]
----
Application pods (SHOWROOM_USERNAME-observability-demo)
  +--------------------------------------------------------+
...
You will deploy a sidecar-mode `OpenTelemetryCollector` CR in your `SHOWROOM_USERNAME-observability-demo` namespace. When this CR exists, the OpenTelemetry Operator automatically injects a sidecar container into any pod in the namespace that carries the annotation `sidecar.opentelemetry.io/inject: "sidecar"`.
...
oc get serviceaccount otel-collector-sidecar \
  -n SHOWROOM_USERNAME-observability-demo
----
+
...
metadata:
  name: sidecar
  namespace: SHOWROOM_USERNAME-observability-demo
spec:
  mode: sidecar
...
[source,bash]
----
oc get opentelemetrycollector sidecar -n SHOWROOM_USERNAME-observability-demo
----
+
.Expected output
[source,text]
...
* ✓ `otel-collector-sidecar` ServiceAccount exists in `SHOWROOM_USERNAME-observability-demo`
...
. Verify you have create access: `oc auth can-i create opentelemetrycollectors -n SHOWROOM_USERNAME-observability-demo`

**Issue**: ServiceAccount `otel-collector-sidecar` not found

**Solution**:
. Check available service accounts: `oc get sa -n SHOWROOM_USERNAME-observability-demo`
...
The `Instrumentation` CR is a template that tells the OpenTelemetry Operator how to configure the auto-instrumentation agent init-container when injecting into pods. You need one per namespace. In this exercise you create it for `SHOWROOM_USERNAME-observability-demo` — it will be used in Exercise 7 when you enable Python auto-instrumentation on the **notifier** service.
...
[source,bash]
----
oc get instrumentation my-instrumentation -n SHOWROOM_USERNAME-observability-demo
----
+
.Expected output
[source,text]
...
* ✓ `my-instrumentation` Instrumentation CR exists in `SHOWROOM_USERNAME-observability-demo`
...
Now you'll activate the OpenTelemetry SDK in all three services. This adds the sidecar injection annotation, the SDK environment variables, and the correct ServiceAccount to each deployment without rebuilding container images.
...
----
+
The annotation value `sidecar` must match the name of the `OpenTelemetryCollector` CR you created in Exercise 2. When the operator sees this annotation on a pod being created, it injects the collector container spec from that CR.
+
.Expected output
[source,text]
----
...
**Solution**:
. Check that the annotation was applied: `oc get deployment frontend -n ${NAMESPACE} -o jsonpath='{.spec.template.metadata.annotations}'`
. Verify the sidecar CR exists: `oc get opentelemetrycollector sidecar -n ${NAMESPACE}`
...
**Solution**:
. View sidecar logs: `oc logs -n ${NAMESPACE} deployment/frontend -c otc-container`
. Common cause: ServiceAccount lacks permissions for `k8sattributes`—verify: `oc auth can-i get pods --as=system:serviceaccount:${NAMESPACE}:otel-collector-sidecar`
. Check central collector reachability: `oc exec -n ${NAMESPACE} deployment/frontend -c otc-container -- wget -qO- http://central-collector-collector.observability-demo.svc:13133/`
...
----
+
The two `backend: HTTP POST` client spans show parallel downstream calls — one to the database service and one to the notifier service. Bar length represents duration. The server span for each service is the parent of that service's outgoing client spans.

. Click a span to expand its attributes:
+
The attributes on any span come from three distinct layers — the application itself, the Go OTel SDK semantic conventions, and the sidecar collector processors. Click spans from each service to compare.
+
...
[NOTE]
====
All TraceQL queries below include `{ .k8s.namespace.name = "SHOWROOM_USERNAME-observability-demo" }` to scope results to your namespace. The Tempo backend is shared across all workshop users, so this filter is required to see only your traces.
====
+
...
[source,text]
----
{ .k8s.namespace.name = "SHOWROOM_USERNAME-observability-demo" && span["baggage.client.platform"] = "web" }
----
+
...
[source,text]
----
{ .k8s.namespace.name = "SHOWROOM_USERNAME-observability-demo" && span["baggage.request.source"] = "workshop-demo" }
----
...
[source,text]
----
{ .k8s.namespace.name = "SHOWROOM_USERNAME-observability-demo" && resource.service.name = "database" && duration > 50ms }
----
+
...
[source,text]
----
{ .k8s.namespace.name = "SHOWROOM_USERNAME-observability-demo" && span["db.sql.table"] = "notes" && duration > 30ms }
----
+
...
[source,text]
----
{ .k8s.namespace.name = "SHOWROOM_USERNAME-observability-demo" && span["db.operation"] = "INSERT" && resource.service.name = "database" }
----
...
[source,text]
----
{ .k8s.namespace.name = "SHOWROOM_USERNAME-observability-demo" && span["note.id"] != "" }
----
+
...
[source,text]
----
{ .k8s.namespace.name = "SHOWROOM_USERNAME-observability-demo" && span["peer.service"] = "database" && span["db.sql.table"] = "notes" }
----
...
[source,text]
----
{ .k8s.namespace.name = "SHOWROOM_USERNAME-observability-demo" && status = error }
----
+
...
[source,logql]
----
{kubernetes_namespace_name="SHOWROOM_USERNAME-observability-demo"} |= "<your-trace-id>"
----
+
...
* ✓ Traces appear in **Observe → Traces** for namespace `SHOWROOM_USERNAME-observability-demo`
...
* ✓ TraceQL query `{ .k8s.namespace.name = "SHOWROOM_USERNAME-observability-demo" && span["baggage.request.source"] = "workshop-demo" }` returns results
* ✓ TraceQL query `{ .k8s.namespace.name = "SHOWROOM_USERNAME-observability-demo" && span["db.sql.table"] = "notes" }` returns backend and database spans
* ✓ A trace ID from the trace view can be found in **Observe → Logs**

**What you learned**: The workshop application uses two complementary instrumentation strategies. The sidecar collector processors automatically enrich spans with Kubernetes metadata — no application code required. The Go OTel SDK adds semantic-convention attributes (`db.*`, `peer.*`, `net.*`) for production-grade observability, business-level attributes (`note.*`, `event.*`) for application-specific queries, and W3C Baggage to propagate contextual metadata across all service boundaries. This combination lets you answer both infrastructure questions ("which pod processed this request?") and application questions ("which note was created and how long was its content?") from a single span.
...
**Solution**:
. Verify traffic reached the app: `oc logs -n SHOWROOM_USERNAME-observability-demo deployment/frontend -c frontend | tail -10`
. Check sidecar export errors: `oc logs -n SHOWROOM_USERNAME-observability-demo deployment/frontend -c otc-container | grep -i "error\|export"`
...
**Solution**:
. Verify all three deployments have sidecars: `oc get pods -n SHOWROOM_USERNAME-observability-demo -o custom-columns='NAME:.metadata.name,CONTAINERS:.spec.containers[*].name'`
. Confirm `OTEL_ENABLED` is set on all deployments: `oc set env deployment/backend -n SHOWROOM_USERNAME-observability-demo --list | grep OTEL`

**Issue**: Spans show no Kubernetes metadata attributes

**Solution**:
. Check ServiceAccount permissions: `oc auth can-i get pods --as=system:serviceaccount:SHOWROOM_USERNAME-observability-demo:otel-collector-sidecar`
. Check `k8sattributes` errors in sidecar logs: `oc logs -n SHOWROOM_USERNAME-observability-demo deployment/frontend -c otc-container | grep -i k8s`
...
[source,logql]
----
{kubernetes_namespace_name="SHOWROOM_USERNAME-observability-demo"} | json
----
+
...
* ✓ **Observe → Logs** shows structured log records from `SHOWROOM_USERNAME-observability-demo` with `traceID` fields
...
. Open **Observe** → **Traces** in the OpenShift console, set namespace to `SHOWROOM_USERNAME-observability-demo`, and click a recent trace.
+
...
oc get instrumentation my-instrumentation \
  -n SHOWROOM_USERNAME-observability-demo -o yaml
----
+
Key fields:
+
...
[source,bash]
----
oc rollout status deployment/notifier -n SHOWROOM_USERNAME-observability-demo
----
...
[source,bash]
----
oc get pods -n SHOWROOM_USERNAME-observability-demo \
  -l app=notifier \
  -o custom-columns='NAME:.metadata.name,CONTAINERS:.spec.containers[*].name'
----
+
...
[source,bash]
----
oc logs -n SHOWROOM_USERNAME-observability-demo \
...
**Solution**:
. Verify the annotation is set: `oc get deployment notifier -n SHOWROOM_USERNAME-observability-demo -o jsonpath='{.spec.template.metadata.annotations}'`
. Confirm the sidecar CR exists: `oc get opentelemetrycollector sidecar -n SHOWROOM_USERNAME-observability-demo`
...
**Solution**:
. Verify `propagators` includes `tracecontext`: `oc get instrumentation my-instrumentation -n SHOWROOM_USERNAME-observability-demo -o jsonpath='{.spec.propagators}'`
. Confirm `OTEL_EXPORTER_OTLP_PROTOCOL=http/protobuf` is injected: `oc exec -n SHOWROOM_USERNAME-observability-demo deployment/notifier -- env | grep OTEL`

**Issue**: No traces appear for the notifier after annotation

**Solution**:
. Check if the Python bootstrap ran: `oc logs -n SHOWROOM_USERNAME-observability-demo deployment/notifier -c notifier | grep -i opentelemetry`
. Verify the sidecar received data: `oc logs -n SHOWROOM_USERNAME-observability-demo deployment/notifier -c otc-container | grep -i "span\|error"`
. Ensure the Instrumentation CR exists in your namespace: `oc get instrumentation my-instrumentation -n SHOWROOM_USERNAME-observability-demo`
