...
----
+
Each downstream service reads this header and records every baggage member as a span attribute prefixed with `baggage.`. This means the same logical context — where the request came from — is searchable as a span attribute on every service in the chain. Filter all traces that originated from the web frontend:
+
[source,text]
----
{ span["baggage.client.platform"] = "web" }
----
+
...
[source,text]
----
{ span["baggage.request.source"] = "workshop-demo" }
----
...
----
+
This filters to only traces where the database service had at least one span exceeding 50ms. Because random delays of up to 60ms are injected in the backend and database services, you should see several results.
+
Query by the specific table the slow operation touched:
+
[source,text]
----
{ span["db.sql.table"] = "notes" && duration > 30ms }
----
+
...
[source,text]
----
{ span["db.operation"] = "INSERT" && resource.service.name = "database" }
----

. Use TraceQL to surface business-level data:
+
The custom `note.*` and `event.*` attributes open the trace store as a queryable record of application events. Find all traces that created a note longer than a given threshold by selecting spans with a content length attribute:
+
[source,text]
----
{ span["note.id"] != "" }
...
[source,text]
----
{ span["peer.service"] = "database" && span["db.sql.table"] = "notes" }
----
...
* ✓ TraceQL query `{ span["baggage.request.source"] = "workshop-demo" }` returns results
* ✓ TraceQL query `{ span["db.sql.table"] = "notes" }` returns backend and database spans
