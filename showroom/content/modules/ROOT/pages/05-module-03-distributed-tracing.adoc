...
image::overview-traces.png[Core distributed tracing concepts: trace, span, context propagation, parent-child relationships, and how tracing reveals bottlenecks invisible to metrics and logs alone]
...
image::opentelemetry-two-tier-collector.png[Two-Tier OpenTelemetry Collector Architecture with Sidecar to Central Fanout and Spanmetrics Connector]
...
* **Rate**: `traces_span_metrics_calls_total` — request count per service and operation
* **Errors**: `traces_span_metrics_calls_total{status_code="STATUS_CODE_ERROR"}` — error count
* **Duration**: `traces_span_metrics_duration_milliseconds_bucket` — latency histogram with configurable buckets
...
[source,text,role="copypaste"]
----
NAME    MANAGEMENT   TEMPO VERSION   READY   AGE
tempo   Managed      2.10.8          True    2d5h
----
+
The `READY` column must be **True** (and `MANAGEMENT` **Managed**) before traces can be ingested. Older operator versions showed `NAME  AGE  CONDITION` with `Ready` instead.
...
* ✓ TempoStack instance shows `READY=True` (Managed, with a `TEMPO VERSION` like `2.10.8`)
...
image::module3-01-tempo-pods.png[Tempo Stack pods all Running in the openshift-tempo-operator namespace]
...
----
NAME               MODE         VERSION
central-collector  deployment   0.152.0-3
----
...
[NOTE]
====
Note that `k8s.pod.uid` is intentionally omitted from the sidecar's `k8sattributes` processor. The `k8s.pod.uid` attribute is unique per pod instance and would cause high cardinality in both Prometheus time series and Loki index labels. For grouping and identifying pods at the application level, `k8s.pod.name` and `k8s.deployment.name` are sufficient.
====
...
[NOTE]
====
If a pod rolled out before the `sidecar` `OpenTelemetryCollector` CR was fully
admitted by the operator's mutating webhook, it may have started without a
sidecar. If `otc-container` does not appear under `INIT` in the next step,
trigger a manual rollout restart:

[source,bash,role="copypaste"]
----
oc rollout restart deployment/frontend deployment/backend -n ${NAMESPACE}
oc rollout restart statefulset/database -n ${NAMESPACE}
----

Then re-run the `oc rollout status` commands above to confirm completion.
====
...
. Verify the Prometheus `/metrics` endpoint is live on each service:
+
The Go services expose a second instrumentation pathway: a Prometheus text-format
`/metrics` endpoint served by the `prometheus/client_golang` library alongside
the OTel SDK. This endpoint is **scraped directly** by Prometheus, bypassing the
OTel sidecar entirely.  HTTP access logs are written to **stdout** as JSON lines
— they are shipped to LokiStack by the cluster-logging pipeline independently
of OTel.
+
[source,bash,role="copypaste"]
----
for app in frontend backend; do
  POD=$(oc get pod -n ${NAMESPACE} -l app=${app} -o jsonpath='{.items[0].metadata.name}')
  PORT=$( [ "$app" = "frontend" ] && echo 8080 || echo 8081 )
  echo "=== ${app} /metrics ==="
  oc exec -n ${NAMESPACE} ${POD} -c ${app} -- \
    curl -sf http://localhost:${PORT}/metrics | head -20
done
POD=$(oc get pod -n ${NAMESPACE} -l app=database -o jsonpath='{.items[0].metadata.name}')
echo "=== database /metrics ==="
oc exec -n ${NAMESPACE} ${POD} -c database -- \
  curl -sf http://localhost:8082/metrics | head -20
----
+
.Expected output (showing the first few Prometheus metrics lines):
+
[source,text,role="copypaste"]
----
# HELP prom_http_requests_total Total HTTP requests handled (Prometheus client SDK).
# TYPE prom_http_requests_total counter
# HELP prom_http_request_duration_seconds HTTP request latency histogram (Prometheus client SDK).
# TYPE prom_http_request_duration_seconds histogram
----
+
[NOTE]
====
You will see **both** the OTel SDK metrics (pushed via OTLP to the sidecar)
and the Prometheus client metrics (scraped via `/metrics`). The two systems
are independent and use different metric names:
+
* `http_requests_total`, `http_request_duration_seconds` — OTel SDK (pushed to sidecar → central collector → COO Prometheus)
* `prom_http_requests_total`, `prom_http_request_duration_seconds` — Prometheus client (scraped directly → COO Prometheus via ServiceMonitor)
+
In Module 1 you queried the OTel-derived metrics in the Perses dashboard.
After this exercise the Prometheus-client metrics are also available as `prom_*`.
====
+
. Confirm the ServiceMonitors are active in your namespace:
+
The Cluster Observability Operator uses `monitoring.rhobs/v1` `ServiceMonitor`
resources — a different API group from `monitoring.coreos.com/v1` used by the
platform Prometheus.  These were shipped as part of the application manifest
you applied in Module 2.  Each carries the label
`monitoring.rhobs/stack: observability-stack` so the pre-deployed
`MonitoringStack` in `observability-demo` selects and scrapes them
automatically.
+
[source,bash,role="copypaste"]
----
oc get servicemonitor -n ${NAMESPACE} -l monitoring.rhobs/stack=observability-stack
----
+
.Expected output
+
...
* ✓ `/metrics` endpoint responds with `prom_http_requests_total` on all three services
* ✓ Three `ServiceMonitor` resources (`monitoring.rhobs/v1`) created in your namespace, labelled `monitoring.rhobs/stack=observability-stack`
...
You also enabled dual instrumentation: the Go SDK pushes OTel metrics, traces, and logs to the sidecar collector (OTLP), while `prometheus/client_golang` exposes Prometheus-format metrics via `/metrics` that are scraped by the COO MonitoringStack using `monitoring.rhobs/v1` ServiceMonitors. HTTP access logs are written as JSON lines to stdout, independently shipped to LokiStack by cluster-logging.
...
(URL path: `/observe/traces`).
+
[NOTE]
====
Use the left navigation **Observe** → **Traces** to reach the Traces interface.
The direct URL path is `/observe/traces`. Navigating to `/monitoring/traces`
redirects to the Alerting page — use the navigation menu rather than manually
typing the path.
====

. Set the required search parameters before running the query:
+
In the **Filter** section, set:
+
* **Tempo instance**: `openshift-tempo-operator/tempo`
* **Tenant**: `dev`
* **Namespace**: `%OPENSHIFT_USERNAME%-observability-demo`
+
Then set:
...
+
The scatter plot should now show individual points, one per trace, with y-axis showing duration in milliseconds. If the chart is empty, confirm you generated traffic in the previous exercise, then click refresh again.
...
image::module3-03-traces-scatter.png[Traces scatter plot showing individual trace points in the OpenShift console Traces UI]
...
image::module3-04-trace-waterfall.png[Trace waterfall showing the frontend → backend → database call chain with timing bars]
...
|`http.request.method` |`POST` (new OTel semconv; older docs show `http.method`)
|`http.response.status_code` |`201` (new semconv; older docs show `http.status_code`)
|`url.path` |Matched route path (for example, `/api/notes`; older docs show `http.route`)
|`server.address` |`frontend` — logical server name
|===
+
NOTE: The frontend *sets* W3C baggage (`client.platform=web`, `request.source=workshop-demo`) for downstream propagation, but records it only on backend/database spans — not on its own server span. To find frontend-originated traces, query `span.baggage.*` (matches the downstream spans in the same trace).
+
...
+
Database spans also carry `http.request.method`, `http.response.status_code`, and `url.path` (new OTel semconv) alongside the custom attributes below.
+
[options="header",cols="45,55"]
|===
|Attribute |Value / description
...
image::module3-05-span-attributes.png[Database span attributes panel showing db.operation, db.sql.table, note.id, and Kubernetes metadata fields]
...
----
+
Each downstream service reads this header and records every baggage member as a span attribute prefixed with `baggage.`. The frontend sets baggage but records it only downstream — so `span.baggage.*` matches backend/database spans, and at trace level finds every frontend-originated trace. This means the same logical context—where the request came from—is searchable as a span attribute on the downstream services in the chain.
+
...
[NOTE]
====
All TraceQL queries below include `{ resource.k8s.namespace.name = "%OPENSHIFT_USERNAME%-observability-demo" }` to scope results to your namespace. The Tempo backend is shared across all workshop users, so this filter is required to see only your traces.
====
+
...
[source,text,role="copypaste"]
----
{ resource.k8s.namespace.name = "%OPENSHIFT_USERNAME%-observability-demo" && span.baggage.client.platform = "web" }
----
+
...
[source,text,role="copypaste"]
----
{ resource.k8s.namespace.name = "%OPENSHIFT_USERNAME%-observability-demo" && span.baggage.request.source = "workshop-demo" }
----
+
[source,text,role="copypaste"]
----
{ resource.k8s.namespace.name = "%OPENSHIFT_USERNAME%-observability-demo" && resource.service.name = "database" && duration > 50ms }
----
+
...
[source,text,role="copypaste"]
----
{ resource.k8s.namespace.name = "%OPENSHIFT_USERNAME%-observability-demo" && span.db.sql.table = "notes" && duration > 30ms }
----
+
...
[source,text,role="copypaste"]
----
{ resource.k8s.namespace.name = "%OPENSHIFT_USERNAME%-observability-demo" && span.db.operation = "INSERT" && resource.service.name = "database" }
----

. Use TraceQL to surface business-level data:
+
The custom `note.*` and `event.*` attributes open the trace store as a queryable record of application events. Find all traces that created a note (`note.id` is a numeric attribute, so compare against a number, not an empty string):
+
[source,text,role="copypaste"]
----
{ resource.k8s.namespace.name = "%OPENSHIFT_USERNAME%-observability-demo" && span.note.id > 0 }
----
+
...
[source,text,role="copypaste"]
----
{ resource.k8s.namespace.name = "%OPENSHIFT_USERNAME%-observability-demo" && span.peer.service = "database" && span.db.sql.table = "notes" }
----
...
[source,text,role="copypaste"]
----
{ resource.k8s.namespace.name = "%OPENSHIFT_USERNAME%-observability-demo" && status = error }
----
+
...
* ✓ Backend/database spans carry `baggage.client.platform=web` and `baggage.request.source=workshop-demo` (set by the frontend, recorded downstream)
* ✓ Spans carry new OTel semconv HTTP attributes (`http.request.method`, `http.response.status_code`, `url.path`)
...
* ✓ TraceQL query `{ resource.k8s.namespace.name = "%OPENSHIFT_USERNAME%-observability-demo" && span.baggage.request.source = "workshop-demo" }` returns results
...
. Visualize the generated span metrics in Perses:
+
The `spanmetrics` connector remote-writes to the COO Prometheus instance, not to the platform Prometheus scraped by **Observe** → **Metrics**. To query these metrics you must use the COO datasource in Perses.
+
Navigate to **Observe** → **Dashboards** and open Perses. Create a new dashboard or open an existing one, then add a **Time Series** panel. Select the **COO Prometheus** datasource and enter the following query to see the request rate per operation derived solely from traces:
+
[source,promql,role="copypaste"]
----
sum by (service_name, span_name) (rate(traces_span_metrics_calls_total{k8s_namespace_name="%OPENSHIFT_USERNAME%-observability-demo"}[5m]))
----
+
To see the p95 latency for backend operations, add a second panel with:
+
[source,promql,role="copypaste"]
----
...
[NOTE]
====
These metrics are not visible in **Observe** → **Metrics** because that view queries the platform Prometheus. The span-derived RED metrics are stored exclusively in the COO Prometheus instance and are only accessible through the COO datasource in Perses dashboards.
====
...
<3> Sends log records as native OTLP/HTTP protobuf to the LokiStack gateway's application tenant endpoint. The `encoding: json` option from the legacy `loki` exporter does not apply here.
...
. Query application-specific custom OTel metrics in Perses:
+
In addition to the automatic `spanmetrics` generated from traces, our application is custom-instrumented with native OpenTelemetry metrics using the Go OTel SDK. These metrics are pushed directly from the application container sidecars to the central collector, which remote-writes them to the COO Prometheus instance.
+
Unlike the standard Prometheus metrics scraped via the `/metrics` endpoint in Module 1, these custom OTel metrics represent dedicated application performance counters that bypass the traditional scraping infrastructure entirely.
+
Add a panel to your Perses dashboard with the **COO Prometheus** datasource to query the OTel counter for frontend requests proxied to the backend:
+
[source,promql,role="copypaste"]
----
sum by (method, path, backend_status) (rate(frontend_requests_proxied_total{}[5m]))
----
+
You can also query backend request processing and notification metrics:
+
[source,promql,role="copypaste"]
----
sum by (route, method, http_status) (rate(backend_requests_processed_total{}[5m]))
sum by (action) (rate(backend_notifications_sent_total{}[5m]))
----

. Verify trace-correlated OTel application logs in the Log Browser:
+
When `OTEL_ENABLED=true` is set, all application log output using the standard `slog` library is bridged directly to the OTel logs SDK via the `otelslog` handler.
+
This means that logs are not only sent as structured events, but they are also automatically decorated with the active `trace_id` and `span_id`!
+
Navigate to **Observe** → **Logs** and run the following query to view OTel-correlated database write events and proxy attempts:
+
[source,logql,role="copypaste"]
----
{kubernetes_namespace_name="%OPENSHIFT_USERNAME%-observability-demo"} |= "proxied to database" | json
----
+
Expand one of the log lines. Notice the `traceID` and `spanID` fields. Clicking on the trace ID will link you directly back to the exact trace waterfall in **Observe** → **Traces**, demonstrating the tight correlation between metrics, logs, and traces.

[NOTE]
====
The `k8sattributes` processor in the central collector intentionally excludes high-cardinality fields such as `k8s.pod.uid` and `k8s.pod.start_time`. These attributes are unique per pod instance and would cause series explosion in Prometheus and excessive Loki label cardinality. The `k8s.pod.name` and `k8s.deployment.name` attributes provide sufficient granularity for debugging and aggregation.
====
...
* ✓ `traces_span_metrics_calls_total` metric is visible in a Perses panel using the COO Prometheus datasource
* ✓ p95 latency histogram query renders in Perses for backend operations
* ✓ Custom application OTel metrics like `frontend_requests_proxied_total` are queryable in Perses
* ✓ **Observe → Logs** shows structured log records from `%OPENSHIFT_USERNAME%-observability-demo` with `traceID` fields
* ✓ OTel log records like `proxied to database` or `note created` are correlated with `traceID` inside the logs console
...
[NOTE]
====
Python zero-code auto-instrumentation is a **Technology Preview (TP)** feature in Red Hat build of OpenTelemetry 3.9 / OpenShift 4.22. Technology Preview features are not intended for production use but provide early access to upcoming capabilities.
====
...
[NOTE]
====
Wait at least 15 seconds after the traffic loop completes before checking the
Traces UI. The sidecar's `BatchSpanProcessor` buffers spans and flushes every
10 seconds (or when the batch reaches 1024 spans). Traces may not appear
immediately after the loop finishes.
====
...
image::module3-06-trace-4hop-notifier.png[Four-hop trace waterfall showing frontend → backend → notifier → database with the notifier span as a child of the backend span]
...
image::module3-07-notifier-span-attributes.png[Notifier span attributes panel showing http.route, http.status_code, service.name, and k8s.pod.name fields]
...
== Exercise 10 (Bonus): Deploy a multi-signal dashboard

One dashboard, three signals. This bonus exercise brings the whole workshop together: a single Perses dashboard in the OpenShift console that shows **OTel spanmetrics** (RED metrics generated from traces and queried from COO Prometheus), **application logs** (queried from LokiStack), and **traces** (queried from Tempo) — for the same application, in the same view.

=== Where the data comes from

The dashboard file you will apply contains two documents:

* A namespaced `PersesDatasource` named `prometheus` pointing at the shared COO MonitoringStack (same URL and `default: true` configuration the GitOps pipeline pre-provisions).
* A namespaced `PersesDashboard` named `multi-signal-dashboard` with four panels.

[NOTE]
====
Loki and Tempo are **not** defined in the file — and they must not be. They already exist as cluster-scoped `PersesGlobalDatasource` resources named `loki` and `tempo`, managed by GitOps in the `coo-monitoring-stack` chart. Global datasources are visible from every namespace, so the dashboard references them directly by plugin kind and resource name. Only the Prometheus datasource is namespaced, because each user namespace needs its own `default: true` datasource pointing at the shared COO MonitoringStack. Applying the file is idempotent: if the `prometheus` datasource already exists, `oc apply` updates it in place; if it is missing, it is created.
====

The two metric panels query OTel spanmetrics — metrics generated from trace spans by the central collector's `spanmetrics` connector (see Exercise 8). The metric names are produced by the central collector configuration in this repository:

* `traces_span_metrics_calls_total` — counter of spans, labeled with `service_name`, `span_name`, `k8s_namespace_name`
* `traces_span_metrics_duration_milliseconds_bucket` — latency histogram buckets, used for a P95 `histogram_quantile` calculation

The log panel uses LogQL against the LokiStack **application** tenant, and the trace panel uses TraceQL against Tempo — both scoped to your namespace.

[options="header",cols="25,25,25,25"]
|===
|Panel |Signal |Datasource (kind/name) |Query type
|Span Call Rate |Metrics (OTel spanmetrics) |`PrometheusDatasource/prometheus` |`PrometheusTimeSeriesQuery`
|Span Latency (P95) |Metrics (OTel spanmetrics) |`PrometheusDatasource/prometheus` |`PrometheusTimeSeriesQuery`
|Application Logs |Logs |`LokiDatasource/loki` (global) |`LokiLogQuery`
|Application Traces |Traces |`TempoDatasource/tempo` (global) |`TempoTraceQuery`
|===

=== Step 1: Deploy the datasource and dashboard

Fetch the manifest from the workshop repository, substitute your namespace, and apply it. The `sed` replacement of `__NAMESPACE__` is the only change needed:

[source,bash,role="copypaste"]
----
NAMESPACE="%OPENSHIFT_USERNAME%-observability-demo"
curl -sL https://raw.githubusercontent.com/cldmnky/observability-workshop/main/docs/perses-multisignal-dashboard.yaml \
  | sed "s/__NAMESPACE__/${NAMESPACE}/g" \
  | oc apply -f -
----

The file contains two documents separated by `---`. A single apply creates or updates both: `persesdatasource/prometheus` and `persesdashboard/multi-signal-dashboard`. If the datasource was already provisioned by GitOps, `oc apply` reports `configured` — the dashboard still gets created.

=== Step 2: Verify the resources

Confirm both namespaced resources exist, and that Loki and Tempo remain cluster-scoped globals:

[source,bash,role="copypaste"]
----
oc get persesdatasource,persesdashboard -n %OPENSHIFT_USERNAME%-observability-demo
----

[source,bash,role="copypaste"]
----
oc get persesglobaldatasource
----

You should see `prometheus` and `multi-signal-dashboard` in your namespace, while `loki` and `tempo` are listed only as global datasources — no namespaced Loki or Tempo datasource was created.

=== Step 3: Open the dashboard in the console

* Navigate to **Observe** → **Dashboards (Perses)** in the left navigation.
* In the dashboard dropdown, select **Multi-Signal Dashboard**.
* The dashboard shows two sections:
** **OTel Spanmetrics (COO Prometheus)** — Span Call Rate and Span Latency (P95) time series
** **Logs and Traces (Loki + Tempo)** — the Application Logs table and the Application Traces table

The metric panels show only the four workshop services (`frontend`, `backend`, `database`, `notifier`) via the `service_name` filter in the PromQL, so they stay readable even though the spanmetrics connector processes spans from all workshop users sharing the central collector.

=== Step 4: No data? Reuse the Exercise 6 traffic generator

If any panel is empty, generate application traffic exactly as in Exercise 6 — the spans, logs, and metrics this dashboard visualizes all come from the same requests:
...
for i in $(seq 1 15); do
  ...
  curl -sk -o /dev/null -X POST "https://$FRONTEND_URL/api/notes" \
    -H "Content-Type: application/json" \
    -d "{\"title\":\"Dashboard test $i\",\"content\":\"Multi-signal\"}"
  sleep 1
done
echo "Traffic generation complete"
----
...
[NOTE]
====
Data appears with different latencies per signal. The spanmetrics connector flushes metrics every 30 seconds, the dashboard refreshes every 30 seconds (`refreshInterval`), LokiStack ships logs on a short delay, and Tempo indexes traces within seconds. Wait 1–2 minutes after the traffic loop before concluding a panel is broken.
====

[NOTE]
====
If logs and traces appear in their panels but the OTel spanmetrics panels remain empty after the wait above, the problem may be the central collector's Prometheus remote-write path rather than the dashboard. Have the facilitator check the central collector for remote-write errors such as `out of order sample` before troubleshooting the dashboard query — generating more traffic will not resolve this failure.
====

=== Verify

* ✓ `oc apply` reports both `persesdatasource/prometheus` and `persesdashboard/multi-signal-dashboard` created or configured
* ✓ `oc get persesdatasource,persesdashboard -n %OPENSHIFT_USERNAME%-observability-demo` shows both resources
* ✓ `loki` and `tempo` exist only as `PersesGlobalDatasource` — no namespaced duplicates were created
* ✓ Dashboard appears in **Observe** → **Dashboards (Perses)** as **Multi-Signal Dashboard**
* ✓ Span Call Rate panel shows per-service, per-method call rates
* ✓ Span Latency (P95) panel shows per-service P95 latency
* ✓ Application Logs panel shows log lines from your namespace
* ✓ Application Traces panel lists traces from your namespace

**What you learned**: A single Perses dashboard can mix signals from three different backends by referencing datasources at different scopes — the namespaced `prometheus` datasource for COO metrics and the global `loki` and `tempo` datasources for logs and traces. The `spanmetrics` connector turns the traces you already emit into Prometheus-queryable RED metrics with no application changes, so the same traffic drives metric, log, and trace panels simultaneously.
