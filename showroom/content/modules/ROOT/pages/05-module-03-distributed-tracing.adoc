...
* **Errors**: `traces_spanmetrics_calls_total{status_code="STATUS_CODE_ERROR"}` — error count
...
----
sum by (service_name, span_name) (rate(traces_spanmetrics_calls_total{k8s_namespace_name="%OPENSHIFT_USERNAME%-observability-demo"}[5m]))
----
+
...
histogram_quantile(0.95,
  sum(rate(traces_spanmetrics_latency_bucket{k8s_namespace_name="%OPENSHIFT_USERNAME%-observability-demo", k8s_deployment_name="backend"}[5m]))
  by (span_name, le)
)
----
+
...
* ✓ `traces_spanmetrics_calls_total` metric is visible in a Perses panel using the COO Prometheus datasource
...
The two metric panels query OTel spanmetrics — metrics generated from trace spans by the central collector's `spanmetrics` connector and renamed by its transform processor (see Exercise 8). The metric names are produced by the central collector configuration in this repository:

* `traces_spanmetrics_calls_total` — counter of spans, labeled with `service_name`, `http_method`, `http_status_code`
* `traces_spanmetrics_latency_bucket` — latency histogram buckets, used for a P95 `histogram_quantile` calculation
