...
[source,bash]
----
oc exec -n {user}-observability-demo deployment/sample-app -c debug -- sh -c \
...
[source,bash]
----
oc get servicemonitor.monitoring.rhobs sample-app-coo -n {user}-observability-demo --show-labels
----
+
              ...
              plugin:
                ...
                spec:
                  datasource:
                    kind: PrometheusDatasource
                    name: {user}-prometheus
                  query: sum(rate(http_requests_total{namespace="{user}-observability-demo"}[5m]))
                    by (pod)
    httpErrorRate:
      kind: Panel
      spec:
              ...
              plugin:
                ...
                spec:
                  datasource:
                    kind: PrometheusDatasource
                    name: {user}-prometheus
                  query: sum(rate(http_requests_total{namespace="{user}-observability-demo",
                    code=~"[45].."}[5m])) by (pod)
    httpDuration:
      kind: Panel
      spec:
              ...
              plugin:
                ...
                spec:
                  datasource:
                    kind: PrometheusDatasource
                    name: {user}-prometheus
                  query: sum(rate(http_request_duration_seconds_sum{namespace="{user}-observability-demo"}[5m]))
                    by (pod) /
                    sum(rate(http_request_duration_seconds_count{namespace="{user}-observability-demo"}[5m]))
                    by (pod)
              ...
              plugin:
                ...
                spec:
                  ...
                  query: histogram_quantile(0.95,
                    sum(rate(http_request_duration_seconds_bucket{namespace="{user}-observability-demo"}[5m]))
                    by (pod, le))
...
+
NOTE: The Perses standalone UI is a shared environment — use your `{user}` prefix when naming dashboards to avoid conflicts with other workshop users.
