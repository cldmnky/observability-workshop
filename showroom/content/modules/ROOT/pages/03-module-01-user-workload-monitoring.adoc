...
[source,bash,role="copypaste"]
----
oc new-project SHOWROOM_USERNAME-observability-demo
----
+
NOTE: If the namespace already exists, you can switch to it with `oc project SHOWROOM_USERNAME-observability-demo`. The namespace has been pre-configured with the necessary monitoring permissions.
...
[source,bash,role="copypaste"]
----
oc exec -n SHOWROOM_USERNAME-observability-demo deployment/sample-app -c debug -- curl -s localhost:8080
----
+
.Expected output
[source,text,role="copypaste"]
...
[source,bash,role="copypaste"]
----
oc exec -n SHOWROOM_USERNAME-observability-demo deployment/sample-app -c debug -- curl -s localhost:8080/metrics | head -20
----
+
...
* ✓ 2 pods running in SHOWROOM_USERNAME-observability-demo namespace
...
[source,bash,role="copypaste"]
----
oc get servicemonitor -n SHOWROOM_USERNAME-observability-demo
----
+
.Expected output
[source,text,role="copypaste"]
...
[source,bash,role="copypaste"]
----
oc exec -n SHOWROOM_USERNAME-observability-demo deployment/sample-app -c debug -- sh -c \
...
* ✓ ServiceMonitor exists in SHOWROOM_USERNAME-observability-demo namespace
...
. Verify ServiceMonitor exists: `oc get servicemonitor -n SHOWROOM_USERNAME-observability-demo`
. Check that service labels match ServiceMonitor selector
. Verify pods are running and exposing metrics: `oc exec -n SHOWROOM_USERNAME-observability-demo deployment/sample-app -c debug -- curl localhost:8080/metrics`
...
oc exec -n observability-demo prometheus-observability-stack-0 -c prometheus -- \
  ...
  grep -E 'sample-app.*SHOWROOM_USERNAME-observability-demo' | head -5
----
+
.Expected output (showing your application is being monitored)
[source,text,role="copypaste"]
----
..."job":"sample-app-coo/sample-app/0","namespace":"SHOWROOM_USERNAME-observability-demo"...
----
+
...
[source,bash,role="copypaste"]
----
oc get servicemonitor.monitoring.rhobs sample-app-coo -n SHOWROOM_USERNAME-observability-demo --show-labels
----
+
              ...
              plugin:
                ...
                spec:
                  datasource:
                    kind: PrometheusDatasource
                    name: prometheus
                  query: sum(rate(http_requests_total{namespace="SHOWROOM_USERNAME-observability-demo"}[5m]))
                    by (pod)
    httpErrorRate:
      kind: Panel
      spec:
              ...
              plugin:
                ...
                spec:
                  datasource:
                    kind: PrometheusDatasource
                    name: prometheus
                  query: sum(rate(http_requests_total{namespace="SHOWROOM_USERNAME-observability-demo",
                    code=~"[45].."}[5m])) by (pod)
    httpDuration:
      kind: Panel
      spec:
              ...
              plugin:
                ...
                spec:
                  datasource:
                    kind: PrometheusDatasource
                    name: prometheus
                  query: sum(rate(http_request_duration_seconds_sum{namespace="SHOWROOM_USERNAME-observability-demo"}[5m]))
                    by (pod) /
                    sum(rate(http_request_duration_seconds_count{namespace="SHOWROOM_USERNAME-observability-demo"}[5m]))
                    by (pod)
              ...
              plugin:
                ...
                spec:
                  ...
                  query: histogram_quantile(0.95,
                    sum(rate(http_request_duration_seconds_bucket{namespace="SHOWROOM_USERNAME-observability-demo"}[5m]))
                    by (pod, le))
...
+
NOTE: The Perses standalone UI is a shared environment — use your `SHOWROOM_USERNAME` prefix when naming dashboards to avoid conflicts with other workshop users.
...
**Solution**:
. Verify the label: `oc get servicemonitor sample-app-coo -n SHOWROOM_USERNAME-observability-demo --show-labels`
...
. To verify it exists: `oc get persesdatasource prometheus -n SHOWROOM_USERNAME-observability-demo`

**Issue**: Dashboard not visible in console

**Solution**:
. Verify PersesDashboard exists: `oc get persesdashboard -n SHOWROOM_USERNAME-observability-demo`
. Check for errors: `oc describe persesdashboard sample-app-dashboard -n SHOWROOM_USERNAME-observability-demo`
...
kind: PrometheusRule
metadata:
  name: sample-app-alerts-SHOWROOM_USERNAME
  namespace: SHOWROOM_USERNAME-observability-demo
  labels:
    app: sample-app
spec:
  ...
  - name: sample-app
    ...
    - alert: LowRequestRate
      expr: sum(rate(http_requests_total{namespace="SHOWROOM_USERNAME-observability-demo"}[5m])) < 0.1
      for: 2m
      labels:
        severity: warning
      annotations:
...
[source,bash,role="copypaste"]
----
oc get prometheusrule -n SHOWROOM_USERNAME-observability-demo
----
...
* ✓ PrometheusRule exists in SHOWROOM_USERNAME-observability-demo namespace
...
**Solution**:
. Verify PrometheusRule exists: `oc get prometheusrule -n SHOWROOM_USERNAME-observability-demo`
