...
image::metrics-overview.png[The 3 pillars of observability (Metrics, Logs, Traces), the practical detect-identify-locate workflow, and the RED/USE/Golden Signals methodologies]
...
[NOTE]
====
In this module you queried `http_requests_total` — metrics generated by the
OpenTelemetry SDK and exported via OTLP.  In **Module 3** you will add a
second instrumentation pathway using `prometheus/client_golang` that exposes
a parallel set of metrics via `/metrics` endpoints, prefixed with `prom_*`
(e.g. `prom_http_requests_total`).  These are scraped directly by the COO
MonitoringStack using `monitoring.rhobs/v1` ServiceMonitors, giving you two
independent metric pipelines from the same application.
====
...
. Verify the pre-created Perses DataSources for Prometheus, Loki, and Tempo:
+
As part of the multi-signal observability setup, three `PersesDatasource` resources (`prometheus`, `loki`, and `tempo`) have been pre-provisioned in your namespace. They connect the built-in Perses dashboard engine in the OpenShift console directly to the underlying metric, log, and trace backends.
+
Check that all three datasources exist:
+
[source,bash,role="copypaste"]
----
oc get persesdatasource -n %OPENSHIFT_USERNAME%-observability-demo
----
+
.Expected output
[source,text]
...
----
+
Inspect the details of the metrics, logs, and tracing datasources. For example, check the `prometheus` datasource (note the standard `perses.dev/v1alpha2` apiVersion):
+
[source,bash,role="copypaste"]
----
...
----
+
The `default: true` field on the `prometheus` datasource means dashboards in your namespace which don't specify a datasource name will automatically use this one.
+
You can similarly inspect the `loki` and `tempo` datasources. They use the cluster's secure internal endpoints with service account-level token credentials (`ocp-sa-bearer`) to safely read application log and distributed trace signals:
+
[source,bash,role="copypaste"]
----
oc get persesdatasource loki -n %OPENSHIFT_USERNAME%-observability-demo -o yaml
oc get persesdatasource tempo -n %OPENSHIFT_USERNAME%-observability-demo -o yaml
----

. Create a Perses dashboard to visualize your metrics:
+
Now create a dashboard that references the datasource. The dashboard YAML is too large to paste directly, so we'll fetch it from the repository:
+
[source,bash,role="copypaste"]
----
NAMESPACE="%OPENSHIFT_USERNAME%-observability-demo"
curl -sL https://raw.githubusercontent.com/cldmnky/observability-workshop/main/docs/perses-dashboard.yaml \
...
* ✓ PersesDatasource `prometheus`, `loki`, and `tempo` exist in your namespace (prometheus is `default: true`)
...
**Solution**:
. Verify the label: `oc get servicemonitor.monitoring.rhobs sample-app-coo -n %OPENSHIFT_USERNAME%-observability-demo --show-labels`
...
**Issue**: PersesDatasource `prometheus`, `loki`, or `tempo` not found

**Solution**:
. All three datasources are pre-provisioned by the workshop setup. If any are missing, contact the workshop facilitator.
. To verify they exist: `oc get persesdatasource -n %OPENSHIFT_USERNAME%-observability-demo`
