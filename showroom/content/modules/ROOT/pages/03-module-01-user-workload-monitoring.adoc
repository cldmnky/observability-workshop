...
* In OCP 4.22+, the metrics query interface is scoped to the currently selected project. Use the **Project** dropdown at the top of the page to select the namespace you want to query.
+
...
[NOTE]
====
In this module you queried `http_requests_total` — standard Prometheus metrics
scraped from the sample application's `/metrics` endpoint by CMO User Workload
Monitoring and the COO MonitoringStack. In **Module 3** you will introduce
the OpenTelemetry SDK and OTLP push protocol to send distributed traces and
derive RED metrics using OpenTelemetry collectors.
====
