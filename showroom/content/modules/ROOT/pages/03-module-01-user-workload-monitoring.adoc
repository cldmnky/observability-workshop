...
. In the OpenShift console, navigate to the project **openshift-user-workload-monitoring** → **Workloads** and verify user workload monitoring is enabled (that the pods are running):
+
...
. Verify the pre-created Perses DataSources:
+
As part of the multi-signal observability setup, a namespaced `PersesDatasource` resource (`prometheus`) has been pre-provisioned in your namespace, along with cluster-scoped `PersesGlobalDatasource` resources (`loki` and `tempo`). They connect the built-in Perses dashboard engine directly to the underlying metric, log, and trace backends.
+
Check that your namespace's Prometheus datasource exists:
+
[source,bash,role="copypaste"]
----
...
----
+
Inspect the details of the Prometheus datasource (note the standard `perses.dev/v1alpha2` apiVersion):
+
[source,bash,role="copypaste"]
----
...
+
You can also inspect the cluster-scoped global datasources for `loki` and `tempo`:
+
[source,bash,role="copypaste"]
----
...
* ✓ PersesDatasource `prometheus` exists in your namespace (marked `default: true`)
* ✓ PersesGlobalDatasource `loki` and `tempo` exist cluster-wide
...
**Solution**:
. The Prometheus datasource is pre-provisioned by the workshop setup. If it is missing, contact the workshop facilitator.
. To verify it exists: `oc get persesdatasource -n %OPENSHIFT_USERNAME%-observability-demo`
. To verify global datasources exist: `oc get persesglobaldatasource`
