...
. Log into the OpenShift console at {openshift_cluster_console_url}[OpenShift Console^]
+
              ...
              plugin:
                ...
                spec:
                  datasource:
                    kind: PrometheusDatasource
                    name: {user}-prometheus
                  query: sum(rate(http_requests_total{namespace="user4-observability-demo"}[5m]))
                    by (pod)
    httpErrorRate:
      kind: Panel
      spec:
              ...
              plugin:
                ...
                spec:
                  datasource:
                    kind: PrometheusDatasource
                    name: {user}-prometheus
                  query: sum(rate(http_requests_total{namespace="user4-observability-demo",
                    code=~"[45].."}[5m])) by (pod)
    httpDuration:
      kind: Panel
      spec:
              ...
              plugin:
                ...
                spec:
                  datasource:
                    kind: PrometheusDatasource
                    name: {user}-prometheus
                  query: sum(rate(http_request_duration_seconds_sum{namespace="user4-observability-demo"}[5m]))
                    by (pod) /
                    sum(rate(http_request_duration_seconds_count{namespace="user4-observability-demo"}[5m]))
                    by (pod)
              ...
              plugin:
                ...
                spec:
                  ...
                  query: histogram_quantile(0.95,
                    sum(rate(http_request_duration_seconds_bucket{namespace="user4-observability-demo"}[5m]))
                    by (pod, le))
...
+
Navigate to the Perses UI at {perses_url}[{perses_url}^]
+
...
for i in $(seq 1 200); do
  ...
  oc exec -n {user}-observability-demo deployment/sample-app -c debug -- curl -s localhost:8080$ep > /dev/null
  sleep $(awk 'BEGIN{srand(); printf "%.2f\n", rand() * 0.9 + 0.1}')
done
----
+
