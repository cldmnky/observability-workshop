...
[source,bash]
----
oc new-project observability-demo
----
...
[source,bash]
----
oc exec -n observability-demo deployment/sample-app -- curl -s localhost:8080/metrics | head -20
----
+
.Sample output
[source,text]
...
* ✓ 2 pods running in observability-demo namespace
...
* ✓ ServiceMonitor exists in observability-demo namespace
...
. Verify ServiceMonitor exists: `oc get servicemonitor -n observability-demo`
. Check that service labels match ServiceMonitor selector
. Verify pods are running and exposing metrics: `oc exec -n observability-demo deployment/sample-app -- curl localhost:8080/metrics`
...
[source,promql]
----
sum(rate(http_requests_total{namespace="observability-demo"}[5m])) by (pod)
----
+
...
[source,promql]
----
sum(http_requests_total{namespace="observability-demo"}) by (pod)
----
...
spec:
  ...
  - name: sample-app
    ...
    - alert: LowRequestRate
      expr: sum(rate(http_requests_total{namespace="observability-demo"}[5m])) < 0.1
      for: 2m
      labels:
        severity: warning
      annotations:
...
[source,bash]
----
oc get prometheusrule -n observability-demo
----
...
* ✓ PrometheusRule exists in observability-demo namespace
...
**Solution**:
. Verify PrometheusRule exists: `oc get prometheusrule -n observability-demo`
