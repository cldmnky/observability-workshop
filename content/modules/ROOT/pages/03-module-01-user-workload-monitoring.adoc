...
**Platform monitoring** (managed by Cluster Monitoring Operator - CMO):
...
**User workload monitoring** (managed by CMO):
...
**Cluster Observability Operator (COO)** (independent operator for advanced use cases):

* Creates namespace-scoped monitoring stacks via MonitoringStack custom resource
* Functions independently of CMO - both can coexist without conflict
* Label-based ServiceMonitor discovery (e.g., `monitoring.rhobs/stack: observability-stack`)
* Deploys Prometheus, Thanos Querier (optional), Alertmanager (optional)
* Supports UI plugins for enhanced dashboards (Perses), logging, and troubleshooting
* Namespace: Can deploy stacks in any namespace (e.g., `observability-demo`)
* Scope: Multi-tenant, team-scoped, or application-specific monitoring

**Key advantages of COO**:

* **Multi-tenancy**: Create separate monitoring stacks per team or namespace
* **Extensibility**: Add custom metrics without losing support
* **Scalability**: Multiple monitoring stacks on a single cluster
* **Flexibility**: Independent release cycle from OpenShift
* **Longer retention**: Supports historical analysis and capacity planning

**When to use each**:

* Use **platform monitoring** for cluster infrastructure (managed by admins, you can't modify it)
* Use **user workload monitoring** for simple application metrics without custom dashboards or long-term retention
* Use **COO** when you need:
  - Custom Perses dashboards and enhanced visualizations
  - Namespace isolation or multi-tenant monitoring
  - Longer data retention periods
  - More configuration flexibility
  - Multiple monitoring stacks for different teams
...
Your cluster has **two independent monitoring systems**:
