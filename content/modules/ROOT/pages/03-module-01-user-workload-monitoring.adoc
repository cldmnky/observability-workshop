...
metadata:
  name: observability-demo-prometheus
...
. Check MonitoringStack has namespaceSelector to discover resources in other namespaces:
+
[source,bash]
----
oc get monitoringstack observability-stack -n observability-demo -o jsonpath='{.spec.namespaceSelector}'
----
+
Should return `{}` (empty selector = all namespaces). If it returns nothing, the MonitoringStack only watches its own namespace.
. Check MonitoringStack resourceSelector: `oc get monitoringstack observability-stack -n observability-demo -o yaml | grep -A5 resourceSelector`
