...
logging-loki   1h    1x.small   static    Ready
----
+
...
. View collector logs: `oc logs -n openshift-logging -l component=collector`
. Verify ClusterLogging resource exists: `oc get clusterlogging -n openshift-logging`
...
[source,logql]
----
{kubernetes_namespace_name="observability-demo"} | json | line_format "{{.kubernetes_pod_name}}: {{.message}}"
----
+
This formats output to show which pod generated each log line.
...
. Check collector is forwarding logs: `oc logs -n openshift-logging -l component=collector`
