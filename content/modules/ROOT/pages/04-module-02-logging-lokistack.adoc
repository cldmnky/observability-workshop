...
oc logs -n {user}-observability-demo statefulset/database --tail=20
----
+
.Sample output
[source,text]
...
[source,logql]
----
{kubernetes_namespace_name="{user}-observability-demo"} |= "404" |json
----
+
...
[source,logql]
----
{kubernetes_namespace_name="{user}-observability-demo", kubernetes_pod_name=~"frontend.*"} |json
----
+
...
[source,logql]
----
{kubernetes_namespace_name="{user}-observability-demo", kubernetes_pod_name=~"frontend.*"} |= "/api/notes" |json
----
...
[source,logql]
----
{kubernetes_namespace_name="{user}-observability-demo", kubernetes_pod_name=~"frontend.*"} |= "path=/ping" |= "status=200" |json
----
+
...
[source,logql]
----
{kubernetes_namespace_name="{user}-observability-demo"} |json | line_format "{{.kubernetes_pod_name}}: {{.message}}"
----
+
...
[source,bash]
----
oc get alertingrule -n {user}-observability-demo
----
...
. Verify namespace: for `application` tenant, AlertingRules must be in your application namespace (e.g. `{user}-observability-demo`), not `openshift-logging`
. Verify the namespace has the `openshift.io/loki-rules=true` label: `oc get namespace {user}-observability-demo --show-labels`
. Check for syntax errors: `oc describe alertingrule high-error-rate -n {user}-observability-demo`
...
. Check Loki rules: `oc get alertingrule -n {user}-observability-demo`
