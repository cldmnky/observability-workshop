...
for i in {1..50}; do
  ...
  oc exec -n {user}-observability-demo deployment/sample-app -c debug -- curl -s localhost:8080/nonexistent > /dev/null
...
[source,bash]
----
oc logs -n {user}-observability-demo deployment/sample-app --tail=20
----
+
.Sample output
[source,text]
...
----
for i in {1..20}; do
  oc exec -n {user}-observability-demo deployment/api-backend -- curl -s localhost:8080 > /dev/null
  sleep 1
done
----
...
[source,logql]
----
{kubernetes_namespace_name="{user}-observability-demo", kubernetes_pod_name=~"sample-app.*"}
----
+
...
[source,logql]
----
{kubernetes_namespace_name="{user}-observability-demo", kubernetes_pod_name=~"sample-app.*"} |= "200" != "404"
----
+
...
----
for i in {1..100}; do
  oc exec -n {user}-observability-demo deployment/sample-app -- curl -s localhost:8080/error > /dev/null 2>&1
  sleep 0.1
done
----
...
[source,promql]
----
rate(http_requests_total{namespace="{user}-observability-demo", code="404"}[5m])
----
+
