...
Armed with metrics from Module 1 and centralized logging from Module 2, your team at Red Hat Observability Inc. can now detect issues and understand what went wrong. But there's still a challenge: "When customers report slow response times, we can see that the API is slow, but we don't know which of our 12 microservices is causing the bottleneck," explains your lead architect. "We need to trace requests end-to-end."

Red Hat Observability Inc.'s application is a complex mesh of microservices. A single customer request might touch 5-8 different services. When performance degrades, metrics show overall slowness, and logs show individual service behavior, but neither reveals where in the chain the delay occurs.
...
* Example: "Query inventory database" (one step in the checkout trace)
...
----
NAME                                   READY   STATUS    RESTARTS   AGE
tempo-instance-compactor-xxxxx         1/1     Running   0          1h
tempo-instance-distributor-xxxxx       1/1     Running   0          1h
tempo-instance-ingester-0              1/1     Running   0          1h
tempo-instance-querier-xxxxx           1/1     Running   0          1h
tempo-instance-query-frontend-xxxxx    1/1     Running   0          1h
----

. Check if Jaeger Query UI is exposed:
+
[source,bash]
----
...
----
NAME                     HOST/PORT                                  
tempo-instance-jaeger    tempo-instance-jaeger-openshift-tempo...
----
...
[source,bash]
----
oc get route tempo-instance-jaeger -n openshift-tempo-operator -o jsonpath='{.spec.host}'
----
+
...
**Solution**:
. Check TempoStack pods: `oc get pods -n openshift-tempo-operator -l app.kubernetes.io/instance=tempo-instance`
. Describe TempoStack: `oc describe tempostack tempo-instance -n openshift-tempo-operator`
...
**Solution**:
. Check if Jaeger Query is enabled in TempoStack
. List all routes: `oc get routes -n openshift-tempo-operator`
. Verify TempoStack has Jaeger configuration: `oc get tempostack tempo-instance -o yaml | grep jaeger`
    ...
    spec:
      ...
      - name: frontend
        ...
        - name: JAEGER_ENDPOINT
          value: "http://tempo-instance-distributor.openshift-tempo-operator.svc:4318/v1/traces"
        - name: OTEL_EXPORTER_OTLP_ENDPOINT
          value: "http://tempo-instance-distributor.openshift-tempo-operator.svc:4318"
---
apiVersion: v1
kind: Service
metadata:
...
[source,bash]
----
oc get route frontend -n tracing-demo -o jsonpath='{.spec.host}'
----
...
* ✓ frontend pod is Running in tracing-demo namespace
...
**Solution**:
. Check pod status: `oc get pods -n tracing-demo`
. View pod logs: `oc logs -n tracing-demo deployment/frontend`
. Verify image pull: `oc describe pod -n tracing-demo -l app=frontend`
. Check for resource constraints: `oc get events -n tracing-demo`

**Issue**: Application route not accessible

**Solution**:
. Verify route exists: `oc get route frontend -n tracing-demo`
. Check service: `oc get svc frontend -n tracing-demo`
. Test from within cluster: `oc exec -n tracing-demo deployment/frontend -- curl localhost:8080`
...
. Check application is sending traces: `oc logs -n tracing-demo deployment/frontend | grep -i trace`
...
[source,promql]
----
histogram_quantile(0.95, rate(http_request_duration_seconds_bucket{namespace="tracing-demo"}[5m]))
----
+
...
* Find Traces
+
Select one of the slow traces for Customer 3.
...
[source,logql]
----
{kubernetes_namespace_name="tracing-demo"} |= "Customer 3"
----
+
...
[source,logql]
----
{kubernetes_namespace_name="tracing-demo"} |= "trace_id" |= "<trace-id-from-jaeger>"
----
+
...
. Check metric labels match: `namespace="tracing-demo"`
. Allow 30-60 seconds for Prometheus scraping
. Verify application exposes metrics: `oc exec -n tracing-demo deployment/frontend -- curl localhost:8080/metrics`
...
[source,bash]
----
oc set env deployment/frontend -n tracing-demo OTEL_TRACES_SAMPLER=parentbased_traceidratio
oc set env deployment/frontend -n tracing-demo OTEL_TRACES_SAMPLER_ARG=0.1
----
+
...
[source,bash]
----
oc rollout restart deployment/frontend -n tracing-demo
oc rollout status deployment/frontend -n tracing-demo
----
...
[source,bash]
----
oc set env deployment/frontend -n tracing-demo OTEL_TRACES_SAMPLER=parentbased_always_on
----
+
...
**Solution**:
. Verify environment variables: `oc set env deployment/frontend -n tracing-demo --list | grep SAMPLER`
. Check pod was restarted: `oc get pods -n tracing-demo`
. View application logs for sampling messages: `oc logs -n tracing-demo deployment/frontend | grep sampling`
...
You successfully implemented distributed tracing with Tempo at Red Hat Observability Inc.
