...
----
+
Two collectors are deployed: a **central-collector** (deployment mode, 2 replicas) that receives telemetry from applications, and a **sidecar** collector injected into application pods.
...
[source,logql]
----
{kubernetes_namespace_name="observability-demo"} | json | line_format "{{.trace_id}} - {{.message}}"
----
+
