...
. Verify the collector instance deployed via GitOps:
+
[source,bash]
----
oc get opentelemetrycollector -n openshift-opentelemetry-operator
----
+
.Expected output
[source,text]
...
otel-collector   deployment   0.x.0     2/2
----
...
[source,bash]
----
oc get pods -n openshift-opentelemetry-operator -l app.kubernetes.io/component=opentelemetry-collector
----
+
.Expected output
[source,text]
----
NAME                              READY   STATUS    RESTARTS   AGE
otel-collector-xxxxx              1/1     Running   0          1h
otel-collector-xxxxx              1/1     Running   0          1h
----
...
[source,bash]
----
oc get opentelemetrycollector otel-collector -n openshift-opentelemetry-operator -o yaml
----
+
...
**Solution**:
. Check operator logs: `oc logs -n openshift-opentelemetry-operator -l app.kubernetes.io/name=opentelemetry-operator`
. Verify operator deployment: `oc get deployment -n openshift-opentelemetry-operator`
. Check for resource constraints or image pull issues

**Issue**: OpenTelemetryCollector CRD not found

**Solution**:
. Verify operator installation: `oc get csv -n openshift-opentelemetry-operator`
...
**Solution**:
. View collector status: `oc describe opentelemetrycollector otel-collector -n openshift-opentelemetry-operator`
. Check collector pod logs: `oc logs -n openshift-opentelemetry-operator -l app.kubernetes.io/component=opentelemetry-collector`
...
. Create a new project for the OpenTelemetry demo:
+
[source,bash]
----
    ...
    spec:
      ...
      - name: app
        ...
        - name: OTEL_EXPORTER_OTLP_ENDPOINT
          value: "http://otel-collector-collector.openshift-opentelemetry-operator.svc:4318"
...
[source,bash]
----
DICE_URL=$(oc get route dice-roller -n otel-demo -o jsonpath='{.spec.host}')
...
**Solution**:
. Check if application code is mounted: `oc get configmap dice-roller-code -n otel-demo`
. Verify volume mount in pod: `oc describe pod -n otel-demo -l app=dice-roller`
. Check Python dependencies: `oc logs -n otel-demo deployment/dice-roller`

**Issue**: curl returns connection refused

**Solution**:
. Verify route exists: `oc get route dice-roller -n otel-demo`
. Check service: `oc get svc dice-roller -n otel-demo`
. Test internally: `oc exec -n otel-demo deployment/dice-roller -- curl localhost:8080/health`
. View application logs: `oc logs -n otel-demo deployment/dice-roller`
...
[source,logql]
----
{kubernetes_namespace_name="otel-demo"} |= "Rolled a"
----
+
...
+
Look for trace IDs in the log output:
+
[source,logql]
----
{kubernetes_namespace_name="otel-demo"} | json | line_format "{{.trace_id}} - {{.message}}"
----
+
If trace context propagation is working, logs will include trace IDs.
...
[source,promql]
----
otelcol_receiver_accepted_spans{namespace="openshift-opentelemetry-operator"}
----
+
...
**Solution**:
. Verify collector received traces: Check collector logs `oc logs -n openshift-opentelemetry-operator -l app.kubernetes.io/component=opentelemetry-collector`
. Check application OTLP endpoint: `oc set env deployment/dice-roller -n otel-demo --list | grep OTLP`
. Test collector endpoint: `oc exec -n otel-demo deployment/dice-roller -- curl -v otel-collector-collector.openshift-opentelemetry-operator.svc:4318`
. Verify traces exporter in collector config

**Issue**: No metrics in Prometheus

**Solution**:
. Check if collector exposes Prometheus endpoint: `oc get svc -n openshift-opentelemetry-operator`
. Verify ServiceMonitor exists for collector
. Allow time for Prometheus scraping (30-60 seconds)
. Check metrics endpoint directly: `oc exec -n openshift-opentelemetry-operator deployment/otel-collector -- curl localhost:8889/metrics`

**Issue**: No logs in Loki

**Solution**:
. Verify application is logging: `oc logs -n otel-demo deployment/dice-roller`
...
. Create an Instrumentation resource for automatic injection:
+
[source,bash]
----
cat <<EOF | oc apply -f -
...
spec:
  exporter:
    endpoint: http://otel-collector-collector.openshift-opentelemetry-operator.svc:4318
  propagators:
    - tracecontext
    - baggage
  sampler:
...
[source,bash]
----
oc describe pod -n otel-demo -l app=sample-java-app
----
+
...
[source,bash]
----
oc exec -n otel-demo deployment/sample-java-app -- curl -s localhost:8080
----
...
. Ensure operator is running: `oc get pods -n openshift-opentelemetry-operator`
...
. View operator logs: `oc logs -n openshift-opentelemetry-operator -l app.kubernetes.io/name=opentelemetry-operator`

**Issue**: Pod fails to start after injection

**Solution**:
. Check init container logs: `oc logs -n otel-demo <pod> -c opentelemetry-auto-instrumentation`
...
[source,bash]
----
oc get opentelemetrycollector otel-collector -n openshift-opentelemetry-operator -o yaml | less
----
...
spec:
  ...
  config: |
    ...
    exporters:
      ...
      otlp/tempo:
        endpoint: tempo-instance-distributor.openshift-tempo-operator.svc:4317
        tls:
          insecure: true
...
[source,bash]
----
oc get pods -n otel-demo -l app.kubernetes.io/component=opentelemetry-collector -w
----
...
[source,bash]
----
oc set env deployment/dice-roller -n otel-demo \
  OTEL_EXPORTER_OTLP_ENDPOINT=http://custom-collector-collector.otel-demo.svc:4318
----
...
[source,bash]
----
oc rollout restart deployment/dice-roller -n otel-demo
oc rollout status deployment/dice-roller -n otel-demo
----
...
[source,bash]
----
oc logs -n otel-demo -l app.kubernetes.io/component=opentelemetry-collector --tail=50
----
+
...
**Solution**:
. Check configuration syntax: `oc describe opentelemetrycollector custom-collector -n otel-demo`
. View collector pod logs: `oc logs -n otel-demo -l app.kubernetes.io/component=opentelemetry-collector`
...
**Solution**:
. Verify application endpoint updated: `oc set env deployment/dice-roller -n otel-demo --list | grep OTLP`
. Test collector endpoint: `oc exec -n otel-demo deployment/dice-roller -- curl -v custom-collector-collector.otel-demo.svc:4318`
. Check collector service exists: `oc get svc custom-collector-collector -n otel-demo`
