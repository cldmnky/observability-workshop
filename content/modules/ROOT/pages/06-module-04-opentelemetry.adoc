...
. Add the OTEL environment variables to all three deployments:
+
[source,bash]
----
...
deployment.apps/database updated
----
...
deployment.apps/database patched
----

. Set the ServiceAccount on all three deployments so the injected sidecar can call the Kubernetes API:
+
[source,bash]
----
for app in frontend backend database; do
...
deployment.apps/database serviceaccount updated
----
