...
After implementing metrics, logging, and tracing in the previous modules, a new challenge emerges: each service uses different instrumentation libraries. Some services use Prometheus client libraries, others use custom logging frameworks, and tracing requires yet another SDK. This creates maintenance overhead and inconsistent data quality.

Your organization needs a unified approach to observability instrumentation that works consistently across all services, regardless of programming language or framework.
...
* **Standardization**: 1 instrumentation approach across all services
...
[source,bash]
----
oc get opentelemetrycollector -n {user}-observability-demo
----
+
.Expected output
[source,text]
...
[source,bash]
----
oc get pods -n {user}-observability-demo -l app.kubernetes.io/component=opentelemetry-collector
----
+
.Expected output
[source,text]
...
[source,bash]
----
oc get opentelemetrycollector central-collector -n {user}-observability-demo -o yaml
----
+
...
[source,bash]
----
oc get instrumentation -n {user}-observability-demo
----
+
.Expected output
[source,text]
...
----
+
This Instrumentation resource enables auto-instrumentation injection for applications deployed in the `{user}-observability-demo` namespace.
...
**Solution**:
. View collector status: `oc describe opentelemetrycollector central-collector -n {user}-observability-demo`
. Check collector pod logs: `oc logs -n {user}-observability-demo -l app.kubernetes.io/component=opentelemetry-collector`
...
. The `{user}-observability-demo` namespace already exists (created by GitOps). Verify it:
+
[source,bash]
----
oc project {user}-observability-demo
----
  ...
  template:
    ...
    spec:
      ...
      - name: app
        ...
        command: ["/bin/bash", "-c"]
        args:
        - "pip install --quiet flask opentelemetry-distro opentelemetry-exporter-otlp-proto-grpc &&
          opentelemetry-bootstrap -a install &&
          opentelemetry-instrument flask run --host=0.0.0.0 --port=8080"
        ports:
        - containerPort: 8080
          name: http
        env:
        ...
        - name: OTEL_EXPORTER_OTLP_ENDPOINT
          value: "http://central-collector-collector.{user}-observability-demo.svc:4318"
...
[source,bash]
----
oc get pods -n {user}-observability-demo -w
----
+
...
[source,bash]
----
DICE_URL=$(oc get route dice-roller -n {user}-observability-demo -o jsonpath='{.spec.host}')
...
**Solution**:
. Check if application code is mounted: `oc get configmap dice-roller-code -n {user}-observability-demo`
. Verify volume mount in pod: `oc describe pod -n {user}-observability-demo -l app=dice-roller`
. Check Python dependencies: `oc logs -n {user}-observability-demo deployment/dice-roller`

**Issue**: curl returns connection refused

**Solution**:
. Verify route exists: `oc get route dice-roller -n {user}-observability-demo`
. Check service: `oc get svc dice-roller -n {user}-observability-demo`
. Test internally: `oc exec -n {user}-observability-demo deployment/dice-roller -- curl localhost:8080/health`
. View application logs: `oc logs -n {user}-observability-demo deployment/dice-roller`
...
[source,logql]
----
{kubernetes_namespace_name="{user}-observability-demo"} |= "Rolled a"
----
+
...
[source,logql]
----
{kubernetes_namespace_name="{user}-observability-demo"} |= "trace_id"
----
+
...
[source,promql]
----
otelcol_receiver_accepted_spans{namespace="{user}-observability-demo"}
----
+
...
**Solution**:
. Verify collector received traces: Check collector logs `oc logs -n {user}-observability-demo -l app.kubernetes.io/component=opentelemetry-collector`
. Check application OTLP endpoint: `oc set env deployment/dice-roller -n {user}-observability-demo --list | grep OTLP`
. Test collector endpoint: `oc exec -n {user}-observability-demo deployment/dice-roller -- curl -v central-collector-collector.{user}-observability-demo.svc:4318`
. Verify traces exporter in collector config

**Issue**: No metrics in Prometheus

**Solution**:
. Check if collector exposes Prometheus endpoint: `oc get svc -n {user}-observability-demo`
. Verify ServiceMonitor exists for collector
. Allow time for Prometheus scraping (30-60 seconds)
. Check metrics endpoint directly: `oc exec -n {user}-observability-demo deployment/central-collector-collector -- curl localhost:8889/metrics`

**Issue**: No logs in Loki

**Solution**:
. Verify application is logging: `oc logs -n {user}-observability-demo deployment/dice-roller`
...
[source,bash]
----
oc get instrumentation demo-instrumentation -n {user}-observability-demo -o yaml
----
+
...
[source,bash]
----
oc describe pod -n {user}-observability-demo -l app=sample-java-app
----
+
...
[source,bash]
----
oc exec -n {user}-observability-demo deployment/sample-java-app -- curl -s localhost:8080
----
...
**Solution**:
. Check init container logs: `oc logs -n {user}-observability-demo <pod> -c opentelemetry-auto-instrumentation`
...
[source,bash]
----
oc get opentelemetrycollector central-collector -n {user}-observability-demo -o yaml | less
----
...
[source,bash]
----
oc get pods -n {user}-observability-demo -l app.kubernetes.io/component=opentelemetry-collector -w
----
...
[source,bash]
----
oc set env deployment/dice-roller -n {user}-observability-demo \
  OTEL_EXPORTER_OTLP_ENDPOINT=http://custom-collector-collector.{user}-observability-demo.svc:4318
----
...
[source,bash]
----
oc rollout restart deployment/dice-roller -n {user}-observability-demo
oc rollout status deployment/dice-roller -n {user}-observability-demo
----
...
[source,bash]
----
oc logs -n {user}-observability-demo -l app.kubernetes.io/component=opentelemetry-collector --tail=50
----
+
...
**What you learned**: The OpenTelemetry Collector is a flexible telemetry pipeline. Processors transform telemetry (add attributes, filter, sample). Multiple exporters enable sending data to multiple backends simultaneously.
...
**Solution**:
. Check configuration syntax: `oc describe opentelemetrycollector custom-collector -n {user}-observability-demo`
. View collector pod logs: `oc logs -n {user}-observability-demo -l app.kubernetes.io/component=opentelemetry-collector`
...
**Solution**:
. Verify application endpoint updated: `oc set env deployment/dice-roller -n {user}-observability-demo --list | grep OTLP`
. Test collector endpoint: `oc exec -n {user}-observability-demo deployment/dice-roller -- curl -v custom-collector-collector.{user}-observability-demo.svc:4318`
. Check collector service exists: `oc get svc custom-collector-collector -n {user}-observability-demo`
...
You successfully implemented OpenTelemetry for unified observability.
...
Continue to the xref:07-conclusion.adoc[Workshop Completion] page to review your achievements and next steps.
